...
|---|---|---|---|---:|---|
| R-01 | Rich executable objects: freehand paths, geometry, callouts, pictograms, characters, media, lists, charts, technical frames, groups, masks, anchors, transforms, layers, and state | V2 typed scene graph and Slice 3A immutable, seek-independent state kernel exist. Slice 3B renders line/rectangle/rounded rectangle/ellipse/polygon/text/list to SVG and PNG; other object types and live preview/export still fail closed | `schemas/executable-layout-v2.schema.json`; `contracts_v2.py`; `v2_state.py`; `v2_svg.py`; focused raster tests | 1–3 | Partial compositor; full object matrix pending |
| R-02 | One canonical executable action vocabulary from storyboard through export | V2 strict actions and Slice 3A execution for ten state verbs exist; every other verb fails before frame output; full action matrix and export remain pending | `schemas/visual-plan-v2.schema.json`; `v2_state.py`; runtime semantics; Phase 3 per-action renders | 1, 3 | Partial kernel; full execution pending |
...
| R-10 | Object and relationship motion explains flow, copying, synchronization, grouping, replacement, failure, causality, and emphasis | Slice 3A defines deterministic move/scale/rotate state and random-seek parity; relationship animation and rendered explanatory motion remain missing | movement/state runtime; purpose field; deterministic motion fixtures; seek parity | 3, 6 | Partial kernel; visual motion pending |
