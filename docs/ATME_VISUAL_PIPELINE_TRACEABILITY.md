...
| R-11 | Explicit attention target, secondary context, dimming, isolation, progressive disclosure, and reading/listening priority | Slice 3J adds one explicit highlight target action on supported mark/text objects; Slice 3I provides bounded list disclosure. Secondary-context management, dim/isolate, reading/listening priority, and a choreography validator remain missing | `v2_state.py`; `v2_svg.py`; `test_v2_highlight_action.py`; attention schema/runtime; conflict validator; attention golden scenes | 6 | Partial; complete choreography missing |
