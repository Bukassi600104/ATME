...
|---|---|---|---|---:|---|
| R-01 | Rich executable objects: freehand paths, geometry, callouts, pictograms, characters, media, lists, charts, technical frames, groups, masks, anchors, transforms, layers, and state | V2 state kernel, five geometric marks, authored underline/highlight strokes, text/list, bounded freehand paths, and same-board anchor-bound semantic arrows rasterize. Slice 3C composes checksum-verified original icon/pictogram/character/device/document/chart/terminal illustrations from the pinned registry; arbitrary media, richer objects, and live preview/export still fail closed | `schemas/executable-layout-v2.schema.json`; `contracts_v2.py`; `v2_state.py`; `v2_svg.py`; `v2_path.py`; `test_v2_emphasis_marks.py`; `test_v2_freehand.py`; `test_v2_connector_svg.py`; `test_v2_registry_illustrations.py` | 1–3 | Partial compositor; full object matrix pending |
| R-02 | One canonical executable action vocabulary from storyboard through export | V2 strict actions and Slice 3A state verbs exist; Slice 3D renders draw/write semantics, Slice 3G executes exact-binding connect/disconnect on supported arrows, Slice 3I reveals ordered list items, and Slices 3J–3L add bounded highlight/cross-out/dim target actions. Non-list progressive reveal and other unsupported verbs remain blocked; full action matrix and export are pending | `schemas/visual-plan-v2.schema.json`; `contracts_v2.py`; `v2_state.py`; `v2_svg.py`; `test_v2_connection_actions.py`; `test_v2_progressive_list.py`; `test_v2_highlight_action.py`; `test_v2_cross_out_action.py`; `test_v2_dim_action.py`; runtime semantics | 1, 3 | Partial kernel and compositor; full execution pending |
...
| R-10 | Object and relationship motion explains flow, copying, synchronization, grouping, replacement, failure, causality, and emphasis | Slice 3A defines deterministic move/scale/rotate state; Slice 3D renders stroke/glyph construction; Slices 3F–3G render and connect/disconnect one semantic arrow as endpoints move. Multi-object flow, copying, synchronization, grouping/replacement/failure/causality, and broader explanatory motion remain missing | movement/state runtime; `test_v2_connector_svg.py`; `test_v2_connection_actions.py`; purpose field; deterministic motion fixtures; seek parity | 3, 6 | Partial connector motion; broader semantics pending |
| R-11 | Explicit attention target, secondary context, dimming, isolation, progressive disclosure, and reading/listening priority | Slice 3J adds explicit highlight, Slice 3K non-destructive cross-out, and Slice 3L temporary dim on supported mark/text objects; Slice 3I provides bounded list disclosure. Automatic secondary-context selection, isolate, reading/listening priority, and a choreography validator remain missing | `v2_state.py`; `v2_svg.py`; `test_v2_highlight_action.py`; `test_v2_cross_out_action.py`; `test_v2_dim_action.py`; attention schema/runtime; conflict validator; attention golden scenes | 6 | Partial; complete choreography missing |
