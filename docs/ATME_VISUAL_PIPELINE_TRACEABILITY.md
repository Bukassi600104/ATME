...
| R-10 | Object and relationship motion explains flow, copying, synchronization, grouping, replacement, failure, causality, and emphasis | Slice 3A defines deterministic move/scale/rotate state; Slice 3D renders stroke/glyph construction; Slice 3F draws and rebinds a single semantic arrow as endpoints move. Multi-object flow, copying, synchronization, grouping/replacement/failure/causality, and broader explanatory motion remain missing | movement/state runtime; `test_v2_connector_svg.py`; purpose field; deterministic motion fixtures; seek parity | 3, 6 | Partial connector motion; broader semantics pending |
