...
| R-02 | One canonical executable action vocabulary from storyboard through export | V2 strict actions and Slice 3A state verbs exist; Slice 3D renders draw/write semantics, Slice 3G executes exact-binding connect/disconnect on supported arrows, Slice 3I reveals ordered list items, Slices 3J–3M add highlight/cross-out/dim/isolate attention actions, and Slice 3N adds bounded hold/cut/pan/zoom/reframe camera mechanics. Non-list progressive reveal and other unsupported verbs remain blocked; full action matrix and export are pending | `schemas/visual-plan-v2.schema.json`; `contracts_v2.py`; `v2_state.py`; `v2_svg.py`; `v2_camera.py`; `test_v2_connection_actions.py`; `test_v2_progressive_list.py`; `test_v2_highlight_action.py`; `test_v2_cross_out_action.py`; `test_v2_dim_action.py`; `test_v2_isolate_action.py`; `test_v2_camera_action.py`; runtime semantics | 1, 3 | Partial kernel and compositor; full execution pending |
