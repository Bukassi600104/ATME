...
|---|---|---|---|---:|---|
| R-01 | Rich executable objects: freehand paths, geometry, callouts, pictograms, characters, media, lists, charts, technical frames, groups, masks, anchors, transforms, layers, and state | V2 typed scene graph and Slice 3A immutable, seek-independent object-state kernel exist; no v2 pixels, full 35-type compositor, or live preview/export yet | `schemas/executable-layout-v2.schema.json`; `contracts_v2.py`; `v2_state.py`; Phase 3 primitive renders | 1–3 | State kernel complete; compositor pending |
